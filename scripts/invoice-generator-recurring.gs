 ...
 *    - 定期実行の場合: トリガーを設定して毎月1日の0:00〜1:00に自動実行
 *
 * 【初回実行時の注意】
 ...
 *
 * 【定期実行の設定】
 * - 毎月1日の深夜に自動実行されるよう設定してください
 * - 実行時に前月（実行日の前月）のデータが自動取得されます
 * - 例: 11月1日実行 → 10月1日〜10月31日のデータを取得
 */
...
/**
 * 前月の開始日と終了日を計算する関数
 * 【機能】実行日の前月の1日から月末までの日付を取得
 * 【初心者向け説明】
 * - この関数は実行された日の前月の期間を自動で計算します
 * - 例: 11月1日に実行 → 10月1日〜10月31日を返す
 * - 例: 3月15日に実行 → 2月1日〜2月28日(または29日)を返す
 *
 * @returns {Object} { startDate: "YYYY-MM-DD", endDate: "YYYY-MM-DD" }
 */
function getPreviousMonthDateRange() {
  console.log("📅 [開始] 前月の日付範囲を計算します");
  ...
  // 前月の最終日を計算
  // 翌月の0日目を指定すると前月の最終日になる
  const lastDayOfPreviousMonth = new Date(previousYear, previousMonth + 1, 0);

  // YYYY-MM-DD形式にフォーマット
  const startDate = Utilities.formatDate(firstDayOfPreviousMonth, "JST", "yyyy-MM-dd");
  const endDate = Utilities.formatDate(lastDayOfPreviousMonth, "JST", "yyyy-MM-dd");

  console.log(`  ✅ 計算結果: ${startDate} 〜 ${endDate}`);
  console.log("📅 [完了] 前月の日付範囲計算完了\n");
...
function createInvoices() {
  ...
  invoiceDataRows.forEach((invoice, index) => {
    ...
    // 前月の日付範囲を動的に計算
    // 毎月1日に実行されることを想定し、前月のデータを取得
    const dateRange = getPreviousMonthDateRange();
    const startDate = dateRange.startDate;
    const endDate = dateRange.endDate;

    // 日付から年/月/日形式の項目名を生成（例: システム開発費(2025/10/01 ~ 2025/10/31)）
    ...
    if (!invoice.notion_user_id || invoice.notion_user_id.trim() === "") {
      ...
      try {
        console.log(`  📊 対象期間: ${startDate} 〜 ${endDate}（前月）`);
